...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="admin_permission_add('添加权限节点','{{url('add2')}}','','310')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加权限节点</a></span></div>
	<table class="table table-border table-bordered table-bg">
		...
		@foreach($str as $value)
			<tr class="text-c">
				<td><input type="checkbox" value="{{$value->auth_id}}" name=""></td>
				<td>{{$value->auth_id}}</td>
				<td>{{$value->qx_id}}</td>
				<td>{{$value->auth_name}}</td>
				<td>{{$value->auth_action}}</td>
				<td>{{$value->auth_controller}}</td>
